...
We've now added a custom ```learn(·)``` method that does basic cross-entropy and passed our ```Model``` into ```Agent=Run.Model```, overriding the default Agent (which also does cross-entropy). For more sophisticated optimization schemes, we may optimize directly within the ```learn(·)``` method and not return a loss. ```replay``` allows us to sample batches. ```logger``` allows us to keep track of metrics. 
