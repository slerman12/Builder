...
In this example, we use the Dataset ```CIFAR10```. There are many [built-in](#built-ins) datasets, architectures, domains, environments, and so on, such as ```Dataset=CIFAR10```. The default domain is classification and can be changed with the ```task=``` flag.
