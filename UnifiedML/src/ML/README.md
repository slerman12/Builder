...
You can include a ```train=``` boolean arg to your custom Dataset to use the same Dataset for both training and testing s.t. it is toggled between the two modes or pass in a different custom test Dataset via ```TestDataset=``` and the same syntax.
