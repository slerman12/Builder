...
Valid ML objects include not just instantiated objects (e.g. ```Model='nn.Linear(3 * 32 * 32, 10)'```), but classes as well (e.g. ```Model=nn.Linear``` or another [custom architecture](#tutorials) of your own for example). Some arguments, such as shape parameters (```in_features```, ```out_features```), can be automatically inferred. However, for more complex class configurability, various syntax possibilities exist to define the hyper-parameterization. Below are outlined those grammars an ways of defining complex and highly configurable ML structures.
